...
\begin{theorem}
    ...
    \begin{enumerate}
        \item $S \subseteq \tilde{S} \subseteq V \implies 
        \begin{cases}
            \tilde{S}^{\perp} \leq S^{\perp} \leq V^*  \\ 
            \left< S \right>^{\perp} = S^{\perp}
        \end{cases}$
...
\begin{example}
    Let $W \leq \mathbb{R}^5$, we want to find a basis for $W^{\perp}$, where 
    \[
        W = span \{ 
            \begin{pmatrix} 
              1 \\ -1 \\ 0 \\ 0 \\ 0  
    ...
    \textbf{Option 1}: Extend basis for $W$ to basis for $V$, taking the duals gives a basis for the orthogonal complement. \\
    ...
    Extend $\beta_W$ to basis for $V$, say 
    \[
        S = \{ 
            \begin{pmatrix} 
              1 \\ -1 \\ 0 \\ 0 \\ 0  
    ...
    Then, 
    \[
        \{ s_4^*, s_4^* \} , \text{ where } s_4 = e_4, s_5 = e_5
    \]
    ...
    \begin{align*}
        W^{\perp} & = \left<T\right>^{\perp}  \\
        & = T^{\perp} \\  
        &= \{  f : (\mathbb{R}^5)^*: f(t_1) = f(t_2) = f(t_3) = 0 \} 
    \end{align*}

    Let $f(v)$ have the form 
    \[
        f(v) = \begin{pmatrix} 
          a \\ b \\ c \\ d \\ e  
        \end{pmatrix} \cdot v 
    ...
    Hence $W^{\perp}$ has basis $ \{ f_1, f_2 \} $ where 
    \[
        f_1(v) = x_3 + x_4, f_2(v) = x_5
    \]
